...
function buildCandidateReasons(booking, options) {
  ...
  if (booking.user?.telegramId && options.telegramIds.has(booking.user.telegramId)) {
    reasons.push('telegram_id_match');
  }

  if (booking.publicId && options.publicIds.has(booking.publicId)) {
    reasons.push('public_id_match');
  }
...
function formatBookingPreview(candidate) {
  const username =
    candidate.user?.registration?.telegramUsername ??
...
function printSummary(candidates, options) {
  const activeCount = candidates.filter((candidate) => ['CREATED', 'SUBMITTED'].includes(candidate.status)).length;
  const internalCount = candidates.filter((candidate) => candidate.cleanupReasons.includes('internal_boutique')).length;
  const explicitCount = candidates.filter((candidate) => (
    candidate.cleanupReasons.includes('telegram_id_match') ||
    candidate.cleanupReasons.includes('public_id_match')
  )).length;

  console.log(`Mode: ${options.apply ? 'apply' : 'dry-run'}`);
  console.log(`Cleanup candidates: ${candidates.length}`);
  console.log(`Active candidates: ${activeCount}`);
  console.log(`Matched by internal boutique: ${internalCount}`);
  console.log(`Matched by explicit filters: ${explicitCount}`);

  if (candidates.length === 0) {
    console.log('No test bookings matched the cleanup criteria.');
    return;
  }
...
async function applyCleanup(candidates) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const detachedDocumentsResult = await tx.userPdf.updateMany({
      where: {
        bookingId: {
          in: bookingIds,
        },
    ...
    return {
      ...
      detachedDocuments: detachedDocumentsResult.count,
...
async function main() {
  ...
  printSummary(candidates, options);
  ...
  console.log(`Detached timers: ${result.detachedTimers}`);
  console.log(`Detached documents: ${result.detachedDocuments}`);
  console.log('Slots are freed automatically because deleted bookings no longer participate in occupancy checks.');
}

main().catch(async (error) => {
  if (logger) {
    logger.error({ err: error }, 'Failed to cleanup test bookings');
  } else {
    console.error('Failed to cleanup test bookings');
    console.error(error);
  }
...
```json
{
  ...
  "scripts": {
    ...
    "cleanup:test-bookings": "node scripts/cleanupTestBookings.js"
  },
  "dependencies": {
...
npm run cleanup:test-bookings -- --dry-run
npm run cleanup:test-bookings -- --apply
```
...
```bash
npm run cleanup:test-bookings -- --dry-run --telegram-id 9586763375
npm run cleanup:test-bookings -- --apply --public-id 8794ed5f-7300-424d-ae39-29e37754e903
```
